...
# GET AND GATHER DATA
data_screen <- read.csv2("./screen_MTurk_180725.csv")
data_screen_itemdisplay <- read.csv2("./screen_MTurk_itemdisplay_180725.csv")
data_screen_itemdisplay$shown = as.POSIXct(strptime(data_screen_itemdisplay$shown, "%Y-%m-%d %H:%M:%S"))

# data_screen_itemdisplay <- data_screen_itemdisplay[data_screen_itemdisplay$session != "ZFII4MoThM7iAyaEXiXR5ZFCuLRQrnJEB1aZDz_3MzhM-PvbZA_ZIXCDZFPdQuLx", ]
...
data_screen_final_relevantOnly[grep("sUP4jv3J", data_screen_final_relevantOnly$session), ]

unique(data_screen_final_relevantOnly$session)

...
access_to_identifiable_data = TRUE # if TRUE, then data will contain identifiable information. These files are not available to the public.
```
...
if (access_to_identifiable_data == TRUE) {
  data_raw_identifiable <- read.csv2("../../Data/Raw_data/data_raw_full_identifiable_180831.csv")
  data_time <- read.csv2("../../Data/data_raw_full_time_180831.csv")

# get rid of test sessions
  data_raw_identifiable <- data_raw_identifiable[!data_raw_identifiable$session %in% remove_test_sessions,]
...
if (access_to_identifiable_data == TRUE) {
  ...
  ip_adress_count <- rle(sort(as.character(data_raw_identifiable$ip_address.sc)))
  ...
  data_raw_identifiable[data_raw_identifiable$ip_address.sc %in% ip_9, ]
  
  # score if suspicous
  ip_largerThanOne <- as.character(ip_address_data[ip_address_data$n >= 2, ]$ip_address)
  data_raw_identifiable$ip_suspicious <- ifelse(data_raw_identifiable$ip_address.sc %in% ip_largerThanOne, 1, 0)
  table(data_raw_identifiable$ip_suspicious)
}
```

```{r create_nonIdentifiable_data, eval=T, echo=F}
data_clean <- data_raw_identifiable %>% select(-starts_with("ip_address", "email", "nickname", "browser"))

```


```{r count_data, eval=T, echo=F}

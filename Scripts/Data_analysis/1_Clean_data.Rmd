...
ip_verification = TRUE # if true, then IP addresses will be checked via ipstack
...
```{r load_count_data, eval=T, echo=F}
## load final data
...
data_clean$vid1_too_short <- ifelse(data_clean$vid1_difftime > 240 | is.na(data_clean$vid1_difftime), FALSE, TRUE)

# for Vid 2 (one selected randomly from 9 alternatives), numbers indicate the length of the video (in sec)
data_clean$vid2_too_short <- ifelse(data_clean$Group.t1 == 1 & data_clean$vid2_difftime > 126, FALSE, 
...
data_clean$att_check_sum_t1 <- rowSums(data_clean[, c("att_check_item_errors_t1", "att_check_video_errors", 
                                                      "vid1_too_short", "vid2_too_short")])
# calculate overall error score on longitudinal data
data_clean$att_check_sum_t1_t4 <- rowSums(data_clean[, c("att_check_item_errors_t1_t4", "att_check_video_errors", 
                                                          "vid1_too_short", "vid2_too_short")])

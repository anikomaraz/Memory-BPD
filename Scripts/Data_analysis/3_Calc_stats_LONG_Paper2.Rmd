...
data_viz_cesd_m <- gather(data = data_viz_cesd, "CESD.t1", "CESD.t2", "CESD.t3", "CESD.t4", key="time", value="cesd")
...
std_vars <- c("panas_pos", "panas_neg", "cesd", "gen_impr")
...
data_mean_sd <- function(varname, groupnames) {
  require(plyr)
...
```{r mean_sd, eval=T}
means <- c("time", "panas_pos_mean", "panas_neg_mean", "cesd_mean", "gen_impr_mean")
sd <- c("time", "panas_pos_sd", "panas_neg_sd", "cesd_sd", "gen_impr_sd")

# for the total sample
data_mean_sd_panas_pos <- data_mean_sd(varname = "panas_pos", groupnames="time")
data_mean_sd_panas_neg <- data_mean_sd(varname = "panas_neg", groupnames=c("time"))
data_mean_sd_cesd <- data_mean_sd(varname = "cesd", groupnames=c("time"))
data_mean_sd_gen_impr <- data_mean_sd(varname = "gen_impr", groupnames=c("time"))
...
data_plot_means_melt <- melt(data_mean_sd[, means], id="time", variable_name = "scale")
rename(data_plot_means_melt, c(value="mean"))
data_plot_means_melt$scale <- str_sub(data_plot_means_melt$scale, start=1, end=-6)

data_plot_sd_melt <- melt(data_mean_sd[, sd], id="time", variable_name = "scale", 
                          value="sd")
rename(data_plot_sd_melt, c(value="sd"))
data_plot_sd_melt$scale <- str_sub(data_plot_sd_melt$scale, start=1, end=-4)

# join mean and sd
data_plot_melt <- full_join(data_plot_means_melt, data_plot_sd_melt, by=c("time", "scale"))
data_plot_melt <- rename(data_plot_melt, c(value.x = "mean", value.y="sd"))
...
# for groups
means_gr <- c("time", "Group_affect", "panas_pos_mean", "panas_neg_mean", "cesd_mean", "gen_impr_mean")
sd_gr <- c("time",  "Group_affect", "panas_pos_sd", "panas_neg_sd", "cesd_sd", "gen_impr_sd")


# calculate mean and sd
data_mean_sd_panas_pos_gr <- data_mean_sd(varname = "panas_pos", groupnames=c("time", "Group_affect"))
data_mean_sd_panas_neg_gr <- data_mean_sd(varname = "panas_neg", groupnames=c("time", "Group_affect"))
data_mean_sd_cesd_gr <- data_mean_sd(varname = "cesd", groupnames=c("time", "Group_affect"))
data_mean_sd_gen_impr_gr <- data_mean_sd(varname = "gen_impr", groupnames=c("time", "Group_affect"))
...
data_plot_means_melt_gr$scale <- str_sub(data_plot_means_melt_gr$scale, start=1, end=-6)

data_plot_sd_melt_gr <- melt(data_mean_sd_gr[, sd_gr], id=c("time", "Group_affect"), variable_name = "scale", 
                          value="sd")
data_plot_sd_melt_gr <- rename(data_plot_sd_melt_gr, c(value="sd"))
data_plot_sd_melt_gr$scale <- str_sub(data_plot_sd_melt_gr$scale, start=1, end=-4)
...
plot_groups <- function(affect = "positive", title="Positive (N=215)", 
  ...
  # plotting
  plot_groups <- ggplot(data=data, aes(x=time, y=mean)) +
    geom_point(size=2, aes(color=scale)) +  
    geom_line(aes(group=scale, colour=scale), size=1.5) +
      scale_color_manual(values=c("purple", "orange", "blue", "red"),  
                        labels=c("Mood \n(positive to negative)",
                             ...
                             "PANAS Negative",
                             "PANAS Positive")) +
    geom_errorbar(aes(ymin=mean-(sd/sqrt(n_group)), ymax=mean+(sd/sqrt(n_group)), color=scale), 

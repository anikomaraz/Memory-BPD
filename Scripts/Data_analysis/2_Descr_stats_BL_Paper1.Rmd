...
data_final_bl <- read.csv2("../../Data/data_final_bl_180910.csv")
...
scales_continous <- c("BPD_screen", "CESD.t1", "PANAS_POS.t1", "PANAS_NEG.t1", "PANAS.t1", 
            ...
            "BPD", "BPD_stress", "BPD_alien", "BPD_control", "BPD_aggression", "BPD_abs1", "BPD_abs2", "EXTR", "GEN_IMPR.t1")
...
scales_continous_group <- c("BPD_screen", "CESD.t1", "PANAS_POS.t1", "PANAS_NEG.t1", "PANAS.t1", 
            "IPIP_openness", "IPIP_conscientiousness", "IPIP_extraversion", "IPIP_agreeableness", "IPIP_neuroticism", 
            "BPD", "BPD_stress", "BPD_alien", "BPD_control", "BPD_aggression", "BPD_abs1", "BPD_abs2", "EXTR", "GEN_IMPR.t1", "Group_affect")
data_scales_continous_group <- data_final_bl[, scales_continous_group]    

df_anova_Gr <- data.frame("P" = numeric(), "F" = numeric(), 
                          "Neu-Pos_diff" = numeric(), "Neu-Pos_p" = numeric(), 
                          "Neg-Pos_diff" = numeric(), "Neg-Pos_p" = numeric(), 
                          "Neg-Neu_diff" = numeric(), "Neg-Neu_p" = numeric())

df_anova_Gr_calc <- 
  for (i in 1:(ncol(data_scales_continous_group)-1)) {
  anovaresult <- anova(aov(data_scales_continous_group[, i] ~ Group_affect, data = data_scales_continous_group))
  ...
  newRow <- list(anovaresult$`Pr(>F)`[1], 
                  ...
                  posthocresult$Group_affect[1, 1], # diff between neu-pos
                  posthocresult$Group_affect[1, 4], # p value for neu-pos
                  posthocresult$Group_affect[2, 1], # diff between neg-pos
                  posthocresult$Group_affect[2, 4], # p value for neg-pos
                  posthocresult$Group_affect[3, 1], # diff between neg-neu
                  posthocresult$Group_affect[3, 4]) # p value for neg-neu
  
        df_anova_Gr[names(data_scales_continous_group[i]), ] <- newRow
  }

print(round(df_anova_Gr, 2))

# closer look at group differences on PANAS
aggregate(data_final_bl[, c("PANAS_NEG.t1", "PANAS_POS.t1", "PANAS.t1")], by=list(data_final_bl$Group_affect), FUN=mean)
...
## save final  data
if (overwrite_data) {
  write.csv2(data_final_bl, file="../../Data/data_final_bl_scales_180913.csv")
}

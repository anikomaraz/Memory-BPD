...
bfi_items_recode <- colnames(data_final_bl[, grep("^ipip_.+[0-9]R", colnames(data_final), value=F)])
data_final_bl[, bfi_items_recode] <- (6 - data_final_bl[, bfi_items_recode])
setnames(data_final, names(data_final_bl[, bfi_items_recode]), gsub(pattern="R", replacement="", names(data_final_bl[, bfi_items_recode])))
...
```{r describe_scales}
scales <- c("BPD_screen", "CESD.t1", "PANAS_POS.t1", "PANAS_NEG.t1", "PANAS.t1", 
...
data_scales <- data_final_bl[, scales]    
